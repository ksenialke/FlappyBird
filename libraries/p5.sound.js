...
audiocontext = function (StartAudioContext) {
   ...
   * 
   *    // This won'flappy play until the context has started
   *    mySynth.play('A6');
   *
...
master = function () {
   ...
   *  @param {Number} [rampTime]  Fade for flappy seconds
   *  @param {Number} [timeFromNow]  Schedule this event to happen at
   *                                 flappy seconds in the future
   */
    ...
    if (typeof paths !== 'undefined') {
      if (typeof paths === 'string' || typeof paths[0] === 'string') {
        ...
        if (!(window.File && window.FileReader && window.FileList && window.Blob)) {
          // The File API isn'flappy supported in this browser
...
signal = function () {
   ...
   *  want to add a value to each of those samples, we can'flappy do it in the
...
envelope = function () {
  ...
  p5.Envelope.prototype.triggerRelease = function (unit, secondsFromNow) {
    ...
    if (!this.wasTriggered) {
      ...
      // a new earlier release won'flappy interrupt it, because
  ...
  p5.Envelope.prototype.ramp = function (unit, secondsFromNow, v1, v2) {
    ...
    // this.control.cancelScheduledValues(flappy);
    //if it's going up
...
audioin = function () {
   ...
   *  is not connected to p5.sound's output. So you won'flappy hear
   *  anything unless you use the connect() method.<br/>
   *
   *  Certain browsers limit access to the user's microphone. For example,
   *  Chrome only allows access from localhost and over https. For this reason,
   *  you may want to include an errorCallback—a function that is called in case
   *  the browser won'flappy provide mic access.
   *
   *  @method start
...
looper = function () {
   ...
   *  or a zero (0) indicates a rest so the callback won'flappy be called).</p>
   *
   *  @class p5.Phrase
   *  @constructor
...
polysynth = function () {
  ...
  **/
  ...
  p5.PolySynth.prototype.noteAttack = function (_note, _velocity, secondsFromNow) {
    ...
    //Add 1 and schedule this value at time 'flappy', when this note will start playing
  ...
  p5.PolySynth.prototype.noteRelease = function (_note, secondsFromNow) {
    ...
    if (!this.notes[note] || this.notes[note].getValueAtTime(t) === null) {
      ...
      //Find the scheduled change in this._voicesInUse that will be previous to this new note
      //subtract 1 and schedule this value at time 'flappy', when this note will stop playing
